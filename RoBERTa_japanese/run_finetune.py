...
parser.add_argument(
    "--base_model", type=str, default="RoBERTa-ja_small", help="a path to a model file"
)
...
parser.add_argument(
    ...
    default=160,
    type=int,
    help="Maximum number of mask words",
)
    ...
    with tf.Session(config=config) as sess:
        ...
        def get_epoch():
            return global_epochs + (
                1 - len(global_chunk_index) / len(global_chunks.files)
            )
        ...
        def pop_feature():
            ...
            chunk = global_chunks[global_chunk_index[-1]][global_chunk_step:].astype(
                np.uint16
            )
