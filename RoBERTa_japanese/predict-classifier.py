...
parser.add_argument(
    "--output_file", type=str, default="predict.csv", help="result csv file to write."
)
...
def main():
    ...
    with tf.Session(config=config) as sess:
        ...
        pd.DataFrame({"id": input_names, "y_true": input_keys, "y_pred": preds}).to_csv(
        ...
        ax = plt.matshow(r, interpolation="nearest", aspect=0.5, cmap="cool")
        for (i, j), z in np.ndenumerate(r):
            if z >= 1000:
                ...
                plt.text(
                    j - 0.33, i, f"{z}", ha="left", va="center", size=9, color="black"
                )
