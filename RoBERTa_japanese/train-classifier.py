...
parser.add_argument(
    "--model", type=str, default="RoBERTa-ja_small", help="pretrained model directory."
)
...
parser.add_argument(
    "--run_name", type=str, default="RoBERTa-ja_classifier", help="save model name."
)
...
def main():
    ...
    keys = [
        f for f in os.listdir(args.input_dir) if os.path.isdir(args.input_dir + "/" + f)
    ]
    keys = sorted(keys)
    num_labels = len(keys)
    input_contexts = []
    ...
    with tf.Session(config=config) as sess:
        ...
        with tf.variable_scope("loss"):
            ...
            def save():
                ...
                print(
                    "Saving",
                    os.path.join(CHECKPOINT_DIR, args.run_name, "model-{}").format(
                        counter
                    ),
                )
                saver.save(
            ...
            def sample_feature(i):
                ...
                _input_ids = [
                    input_contexts[idx] for idx in input_indexs[i * batch_size : last]
                ]
                _input_masks = [
                    [1] * len(input_contexts[idx])
                    + [0] * (max_seq_length - len(input_contexts[idx]))
                ...
                for ep in range(num_epochs):
                    ...
                    for i in prog:
                        (_, v_loss, v_summary) = sess.run(
                            (opt_apply, loss, summaries), feed_dict=sample_feature(i)
                        )
                        ...
                        prog.set_description(
                            "[{ep} | {time:2.2f}] loss={loss:2.2f} avg={avg:2.2f}".format(
                                ep=ep,
